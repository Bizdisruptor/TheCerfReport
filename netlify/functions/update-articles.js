...
async function getFileSHA(token, owner, repo, branch, filePath) {
  ...
  if (res.status !== 200) {
    throw new Error(`GitHub getFileSHA failed: ${res.status} — ${res.body}`);
  }
...
async function getExistingArticles(token, owner, repo, branch, filePath) {
  ...
  // File doesn't exist yet — start from empty list
  if (res.status === 404) return { articles: [], sha: null };

  // Any non-200 status means GitHub returned an error response (auth failure,
  // rate limit, etc.) — fail loudly with the actual message instead of trying
  // to parse the error body as if it were file content.
  if (res.status !== 200) {
    throw new Error(
      `GitHub getExistingArticles failed: HTTP ${res.status} — ${res.body}`
    );
  }
  ...
  try {
    ...
    throw new Error(`GitHub returned non-JSON body: ${res.body.slice(0, 200)}`);
  }

  // Defensive: if GitHub returns a directory listing or any shape without
  // a `content` field, surface a clear error rather than crashing on undefined.
  if (typeof json.content !== "string") {
    throw new Error(
      `GitHub response missing 'content' field. Path may be a directory or oversized file. ` +
      `Keys returned: ${Object.keys(json).join(", ")}`
    );
  }

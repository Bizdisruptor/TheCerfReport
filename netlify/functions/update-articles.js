 ...
 * Now with diagnostic logging to verify token state.
 *
...
function parseRSS(xml) {
  ...
  while ((match = itemRegex.exec(xml)) !== null) {
    ...
    const type = catText.includes("podcast") || catText.includes("audio") ||
      hasItunesDuration || hasEnclosure ? "podcast" : "newsletter";
...
const handler = async () => {
  ...
  // ── DIAGNOSTIC LOGGING (does NOT expose the token value) ─────────────────
  console.log(`[diag] GITHUB_TOKEN type: ${typeof GITHUB_TOKEN}`);
  console.log(`[diag] GITHUB_TOKEN length (raw): ${GITHUB_TOKEN ? GITHUB_TOKEN.length : 'N/A'}`);
  console.log(`[diag] GITHUB_TOKEN length (trimmed): ${GITHUB_TOKEN ? GITHUB_TOKEN.trim().length : 'N/A'}`);
  console.log(`[diag] GITHUB_TOKEN prefix (first 11 chars): "${GITHUB_TOKEN ? GITHUB_TOKEN.slice(0, 11) : 'N/A'}"`);
  console.log(`[diag] GITHUB_REPO_OWNER: "${GITHUB_REPO_OWNER}"`);
  console.log(`[diag] GITHUB_REPO_NAME:  "${GITHUB_REPO_NAME}"`);
  console.log(`[diag] GITHUB_BRANCH:     "${GITHUB_BRANCH}"`);
  ...
  // Auto-trim to handle any whitespace introduced by the env var UI
  ...
  try {
    ...
    return { statusCode: 200, body: `Added ${newItems.length} new article(s). Total: ${merged.length}` };
  } catch (err) {
